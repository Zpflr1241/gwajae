...
상품 목록 조회·검색·필터·정렬·페이지네이션과 등록/수정/삭제(CRUD)를 제공하는 관리자 화면입니다.
일부 파일은 구현이 끝나면 마저 올리도록 하겠습니다.
...
브라우저에서 http://localhost:5173 접속. 프론트의 `/api/*` 요청은 Vite 프록시가 `http://localhost:4000` 으로 전달합니다.

### 학습용 옵션 (백엔드)

서버는 환경변수로 로딩/에러 상태를 연습할 수 있습니다.
